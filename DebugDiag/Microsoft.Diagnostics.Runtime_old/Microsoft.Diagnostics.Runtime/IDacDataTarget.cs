...
internal interface IDacDataTarget
{
	...
	[PreserveSig]
	int ReadVirtual(ulong address, IntPtr buffer, int bytesRequested, out int bytesRead);

	void WriteVirtual(ulong address, [In][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] byte[] buffer, uint bytesRequested, out uint bytesWritten);
	...
	void Request(uint reqCode, uint inBufferSize, IntPtr inBuffer, IntPtr outBufferSize, out IntPtr outBuffer);
}
