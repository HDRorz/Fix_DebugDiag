...
public interface IDebugClient4 : IDebugClient3, IDebugClient2, IDebugClient
{
	[PreserveSig]
	...
	[PreserveSig]
	int OpenDumpFileWide([In][MarshalAs(UnmanagedType.LPWStr)] string FileName, [In] ulong FileHandle);

	[PreserveSig]
	int WriteDumpFileWide([In][MarshalAs(UnmanagedType.LPWStr)] string DumpFile, [In] ulong FileHandle, [In] DEBUG_DUMP Qualifier, [In] DEBUG_FORMAT FormatFlags, [In][MarshalAs(UnmanagedType.LPWStr)] string Comment);

	[PreserveSig]
	int AddDumpInformationFileWide([In][MarshalAs(UnmanagedType.LPWStr)] string FileName, [In] ulong FileHandle, [In] DEBUG_DUMP_FILE Type);

	[PreserveSig]
	int GetNumberDumpFiles(out uint Number);

	[PreserveSig]
	int GetDumpFile([In] uint Index, [Out][MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer, [In] int BufferSize, out uint NameSize, out ulong Handle, out uint Type);

	[PreserveSig]
	int GetDumpFileWide([In] uint Index, [Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, out uint NameSize, out ulong Handle, out uint Type);
}
