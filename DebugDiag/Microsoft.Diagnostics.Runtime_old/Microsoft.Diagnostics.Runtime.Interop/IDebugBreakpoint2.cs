...
public interface IDebugBreakpoint2 : IDebugBreakpoint
{
	...
	[PreserveSig]
	int GetCommandWide([Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, out uint CommandSize);

	[PreserveSig]
	int SetCommandWide([In][MarshalAs(UnmanagedType.LPWStr)] string Command);

	[PreserveSig]
	int GetOffsetExpressionWide([Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, out uint ExpressionSize);
