...
internal interface ISOSDac
{
	...
	[PreserveSig]
	int GetGCHeapList(uint count, [Out][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] ulong[] heaps, out uint pNeeded);

	[PreserveSig]
	int GetGCHeapDetails(ulong heap, out V4HeapDetails details);
