...
internal struct SigParser
{
	...
	{
		if (UncompressData(out data, out var pDataLen))
		{
			SkipBytes(pDataLen);
			return true;
		}
	...
	private bool PeekElemTypeSize(out int pSize)
	{
		...
		if (!sigParser.GetByte(out var data))
		{
			return false;
		}
		switch (data)
	...
	public bool SkipExactlyOne()
	{
		...
		if (!((ClrElementType)etype).IsPrimitive())
		{
			int token;
			switch (etype)
			{
