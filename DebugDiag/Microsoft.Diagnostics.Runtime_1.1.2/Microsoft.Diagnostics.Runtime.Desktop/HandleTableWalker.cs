...
internal class HandleTableWalker
{
	...
	public byte[] V4Request
	{
		get
		{
			...
			IntPtr functionPointerForDelegate = Marshal.GetFunctionPointerForDelegate((Delegate)_mV4Delegate);
	...
	public byte[] V2Request
	{
		get
		{
			...
			IntPtr functionPointerForDelegate = Marshal.GetFunctionPointerForDelegate((Delegate)_mV2Delegate);
			byte[] array = new byte[IntPtr.Size * 2];
			FunctionPointerToByteArray(functionPointerForDelegate, array, 0);
			return array;
		}
	...
	private bool GetMethodTables(ulong obj, out ulong mt, out ulong cmt)
	{
		...
		if (!_runtime.ReadMemory(obj, array, array.Length, out var bytesRead) || bytesRead != array.Length)
		{
			return false;
		}
		if (IntPtr.Size == 4)
