...
internal abstract class RuntimeBase : ClrRuntime
{
	...
	private const int c_maxStackDepth = 1073741824;
	...
	internal ICorDebugProcess CorDebugProcess
	{
		get
		{
			...
			{
				_corDebugProcess = CLRDebugging.CreateICorDebugProcess(base.ClrInfo.ModuleInfo.ImageBase, base.DacLibrary.DacDataTarget, _dataTarget.FileLoader);
			}
	...
	public RuntimeBase(ClrInfo info, DataTarget dataTarget, DacLibrary lib)
	{
		...
		base.DacLibrary = lib;
		_dacInterface = base.DacLibrary.InternalDacPrivateInterface;
		InitApi();
		_dacInterface.Flush();
		...
		if (gCInfo != null)
		{
			...
			CanWalkHeap = gCInfo.GCStructuresValid;
		}
	...
	internal bool GetHeaps(out SubHeap[] heaps)
	{
		...
		if (base.ServerGC)
		{
			...
			for (int i = 0; i < serverHeapList.Length; i++)
			{
				...
				{
					heaps[i] = new SubHeap(svrHeapDetails, i, allocContexts);
		...
		heaps[0] = new SubHeap(wksHeapDetails, 0, allocContexts);
	...
	public override IEnumerable<ulong> EnumerateFinalizerQueueObjectAddresses()
	{
		...
		{
			foreach (ulong item in GetPointersInRange(subHeap.FQRootsStart, subHeap.FQRootsStop))
			{
				if (item != 0L)
				{
					yield return item;
	...
	internal virtual IEnumerable<ClrRoot> EnumerateStackReferences(ClrThread thread, bool includeDead)
	{
		...
		if (num == 0L || stackLimit - num > 1073741824)
		{
			yield break;
		}
		...
		for (ulong stackPtr = num; stackPtr < stackLimit; stackPtr += (uint)PointerSize)
		{
			if (cache.ReadPtr(stackPtr, out var value) && heap.IsInHeap(value) && heap.ReadPointer(value, out var value2))
			{
				...
				{
					yield return new LocalVarRoot(stackPtr, value, clrType, domain, thread, pinned: false, falsePos: true, interior: false, null);
				}
			}
		}
	}
	...
	{
		value = 0;
		if (!ReadMemory(addr, _dataBuffer, 1, out var _))
		{
			return false;
		}
		value = _dataBuffer[0];
	...
	{
		value = 0;
		if (!ReadMemory(addr, _dataBuffer, 4, out var _))
		{
			return false;
		}
	...
	{
		value = 0.0;
		if (!ReadMemory(addr, _dataBuffer, 8, out var _))
		{
			return false;
		}
	...
	{
		value = 0;
		if (!ReadMemory(addr, _dataBuffer, 2, out var _))
		{
			return false;
		}
	...
	public override bool ReadPointer(ulong addr, out ulong value)
	{
		...
		if (!ReadMemory(addr, _dataBuffer, pointerSize, out var _))
		{
			value = 3435973836uL;
			return false;
		}
