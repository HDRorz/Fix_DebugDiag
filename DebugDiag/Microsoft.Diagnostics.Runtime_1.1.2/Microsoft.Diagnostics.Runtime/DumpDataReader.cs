...
internal class DumpDataReader : IDataReader, IDisposable
{
	...
	private string ExtractCab(string file)
	{
		...
		Command command = Command.Run($"expand -F:*dmp {file} {_generatedPath}", options);
		bool flag = false;
	...
	public uint GetPointerSize()
	{
		...
		if (architecture == Architecture.Amd64 || architecture == Architecture.Arm64)
		{
			return 8u;
		}
		return 4u;
	...
	public IList<ModuleInfo> EnumerateModules()
	{
		...
		foreach (DumpModule item2 in _dumpReader.EnumerateModules())
		{
			MINIDUMP_MODULE raw = item2.Raw;
			ModuleInfo item = new ModuleInfo(this, null)
			{
				FileName = item2.FullName,
				ImageBase = raw.BaseOfImage,
				FileSize = raw.SizeOfImage,
				TimeStamp = raw.TimeDateStamp,
				Version = GetVersionInfo(item2)
			};
			list.Add(item);
		}
		_modules = list;
	...
	{
		VS_FIXEDFILEINFO versionInfo = module.Raw.VersionInfo;
