...
/// <summary>
/// This type of analysis rule runs against every thread in every hang dump - 1 thread at a time
/// By default it runs only against *hang* dumps.
/// The user can opt to run this type of hang analysis against all *crash* dumps as well.
/// </summary>
public interface IHangThreadRule : IAnalysisRuleBase
{
	/// <summary>
	...
	/// <param name="thread">Provides access to the thread information for each thread in a dump.</param>
	...
	void RunAnalysisRule(NetScriptManager manager, NetDbgObj debugger, NetDbgThread thread, NetProgress progress);
}
