...
public interface IDebugBreakpoint3 : IDebugBreakpoint2, IDebugBreakpoint
{
	...
	[PreserveSig]
	new unsafe int GetCommandWide([Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* CommandSize);

	[PreserveSig]
	new int SetCommandWide([In][MarshalAs(UnmanagedType.LPWStr)] string Command);

	[PreserveSig]
	new unsafe int GetOffsetExpressionWide([Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* ExpressionSize);
