...
public interface IDebugClient4 : IDebugClient3, IDebugClient2, IDebugClient
{
	...
	[PreserveSig]
	unsafe int GetDumpFile([In] uint Index, [Out][MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* NameSize, [In] ulong* Handle, out uint Type);

	[PreserveSig]
	unsafe int GetDumpFileWide([In] uint Index, [Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* NameSize, [In] ulong* Handle, out uint Type);
}
