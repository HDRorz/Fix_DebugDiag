...
public interface IDebugBreakpoint
{
	...
	[PreserveSig]
	unsafe int GetCommand([Out][MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* CommandSize);

	[PreserveSig]
	int SetCommand([In][MarshalAs(UnmanagedType.LPStr)] string Command);

	[PreserveSig]
	unsafe int GetOffsetExpression([Out][MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer, [In] int BufferSize, [In] uint* ExpressionSize);
